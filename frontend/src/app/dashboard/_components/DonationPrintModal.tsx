...
import { printDonationPDF } from '@/lib/pdf/generateDonationPDF';
import { cn } from '@/lib/utils';
import { getDonationReport } from '@/services/analytics';
...
export function DonationPrintModal({
  open,
  onOpenChange,
  defaultYear,
  defaultMonth,
  onPrint,
}: DonationPrintModalProps) {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const data = await getDonationReport(startDateStr, endDateStr, {
      ...
      printDonationPDF(startDate, endDate, data);
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Print Donation Report</DialogTitle>
            <DialogDescription>
              Select the start and end date for the donation report you want to print.
            </DialogDescription>
          </DialogHeader>
...
export function DonationPrintModalTrigger({
  children,
  className,
  onClick,
}: {
